...
kotlin {
    ...
    iosArm64 {
        // The iOS fat-framework task pairs iosArm64 with iosX64, so the
        // linkage must match the static simulator slices used by Swift export.
        binaries.framework {
            baseName = "Arboard"
            isStatic = true
            xcf.add(this)
    ...
    swiftExport {
        moduleName = "Arboard"
        flattenPackage = "io.github.kotlinmania.arboard"
    }

    android {
        namespace = "io.github.kotlinmania.arboard"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "arboard-kotlin", version.toString())

    pom {
        name.set("arboard-kotlin")
        description.set("Kotlin Multiplatform port of 1Password/arboard - Image and text handling for the OS clipboard")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/arboard-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/arboard-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/arboard-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/arboard-kotlin.git")
        }
    }
}
...
// kotlinc-compiled commonMain through the CodeQL Java agent. The single-target
// K2JVMCompiler dispatch keeps doExecute(...) on the call stack so the agent's
// KotlinInterceptor class-load hook fires (KGP's multiplatform pipeline routes
// around doExecute and produces 0 bytes of TRAP for the analyzer).
    ...
    doFirst {
        ...
        if (sourceFiles.isEmpty()) {
            val sentinelFile = sentinelDir.get().asFile.resolve(
                "io/github/kotlinmania/arboard/CodeqlEmptySentinel.kt",
            )
