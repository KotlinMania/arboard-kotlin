...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Arboard.swiftmodule/`
...
//      `import Arboard` above would fail at compile time.
//
//   2. The static archive `libArboard.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Arboard" }` and
//      `flattenPackage = "io.github.kotlinmania.arboard"` configuration in
...
//      via the `ArboardLibrary` product.
//
...
final class ArboardExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Arboard swift module imported cleanly")
    }
}
