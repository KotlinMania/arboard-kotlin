# Claude Code Project Instructions — arboard-kotlin

## Project Overview

This is **arboard-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`arboard`](https://crates.io/crates/arboard).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/arboard/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/arboard>
